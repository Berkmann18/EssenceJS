...
case 32:break
case 64:break
default:throw new Error("Unvalid binary number")}return r*Math.pow(2,e)*i}function min2dec(t){return 50*t/30}function dec2min(t){return 30*t/50}function toS(t,e){if(t||(t=e?"00:00:00.000":"00:00.000"),isType(t,"String")||(t+=""),t.length>=4&&1==t.indexOf(":"))return toS("0"+t,e)
var i=t.split(":")
if(e){var r,n,s
...
return i?println(simpleTable("Comparison",r)):console.table(r),r}function database(t,e,i,r,n,s){this.name=t||"Database",this.headerRow=isNon(e)?range(0,1,100):e,this.content=isNon(i)?[].fill("..."):i,this.headerCol=r||["Index","Value"],this.admin=n||"Anonymous",this.version=s||1,this.val=new Array(this.headRow.length)
...
this.toString=function(){return"Stream(start="+this.start.toStr(!0)+", formula="+this.formula+", data="+this.data.toStr(!0)+", results="+this.results.toStr(!0)+")"}}function Graph(t,e,i,r,n){this.labels=i||["x","y"],this.name=r||"Graph",this.dimension=e||new Array(this.labels.length).fill(50),this.equation=new Equation(t),this.data=range(0,n,this.dimension[0],n.length()[1])
...
this.txt2print=""},preInit:function(){$G.t1=1e3*$G.t1.getSeconds()+$G.t1.getMilliseconds()},init:function(){$G.t2=new Date,$G.t2=1e3*$G.t2.getSeconds()+$G.t2.getMilliseconds(),$G.t=$G.t2-$G.t1>1e3?($G.t2-$G.t1)/1e3+"s":$G.t2-$G.t1+"ms",Essence.say("Page loaded in %c"+$G.t+"%c","succ","font-style: italic","font-style: none")},time:function(t,e,i){console.log("[%c"+getTimestamp(!0)+"%c] "+t,"color: #f00;","color: #000;",e,i)},sayClr:function(t){4===t.length?console.log("%cr%cg%cb%ca(%c"+t.join(", %c")+"%c)","color: #f00","color: #0f0","color: #00f","color: #00f","color: #000","color: #f00","color: #0f0","color: #00f","color: #00f","color: #000"):console.log("%cr%cg%cb%c(%c"+t.join(", %c")+"%c)","color: #f00","color: #0f0","color: #00f","color: #00f","color: #000","color: #f00","color: #0f0","color: #00f","color: #00f","color: #000")},ask:function(t,e){for(Sys["in"].recording=!0,Essence.say((t||"Is there a problem")+" ? (please type in the window and not the console)","quest");13!=$G.lastKeyPair[1]&&10!=$G.lastKeyPair[1];)Essence.say("waiting for an enter")
return Sys["in"].recording=!1,e&&e(Sys["in"].data.join("")),Sys["in"].data.join("")}},$G={t1:new Date,t2:0,t:null,lastKeyPair:[]},$e=function(t){return new Element(t)}
...
return t},Array.prototype.first=function(t){return isNon(t)?this[0]:this[0]=t},Array.prototype.last=function(t){return isNon(t)?this[this.length-1]:this[this.length-1]=t},Array.prototype.lastIndex=function(){return this.length-1},Array.prototype.midIndex=function(t){return t?Math.floor(this.length/2)-1:Math.floor(this.length/2)},Array.prototype.even=function(){for(var t=[],e=0;e<this.length;e+=2)t.push(this[e])
...
return i?r.join(i):r},Number.prototype.length=function(){if(String(this).indexOf(".")>-1)return[parseInt(String(this).split(".")[0].length),parseInt(String(this).split(".")[1].length)]
