...
return r=conv(i.join("")),r++,e?conv(r,10,2).split(""):conv(r,10,2)}function min2dec(t){return 50*t/30}function dec2min(t){return 30*t/50}function toS(t,e){if(t||(t=e?"00:00:00.000":"00:00.000"),isType(t,"String")||(t+=""),t.length>=4&&1==t.indexOf(":"))return toS("0"+t,e)
var i=t.split(":")
if(e){var r,s,n
return r=parseInt(i[0]),s=parseInt(i[1]),n=parseFloat(i[2]),3600*r+60*s+n.toNDec()}return s=parseInt(i[0]),n=parseFloat(i[1]),60*s+n}function sec2time(t,e){var i=0,r=0,s=t
return e?(s=(t%60).toNDigits(),i=t>=3600?Math.floor(t/3600):0,r=Math.floor((t-s-3600*i)/60),r=0>=r?"00":r.toNDigits(),i=0>=i?"00":i.toNDigits(),i+":"+r+":"+s.get(0,Math.min(4,s.length-1))):(s=(t%60).toNDigits(),r=Math.floor(t/60).toNDigits(),0>=r?s:r+":"+s.get(0,Math.min(4,s.length-1)))}function markConv(t,e,i,r){return(t/e*(i||100)).toNDec(r||2)}function nthroot(t,e,i){for(var r=t/2,s=0;60>s;s++)r+=(t-Math.pow(r,e))/(Math.pow(r+1,e)-Math.pow(r,e))
...
for(var r=0;i>r;r++)primeCheck(e[r],e[i])&&(e[i]="x")}return e.remove("x")}function primeCheck(t,e){return t>1&&e>1&&e%t===0&&e!=t}function getClosestRoot(t,e){t||Essence.say("x must be initiated"),e||(e=2)
var i=0,r=0
...
r++}}return e.trimAll("r")}function occurenceSort(t){t.getOccurences(!0)}function timeUp(t,e,i){var r=new Date,s=0
...
t&&($G.dnM=!$G.dnM),$G.dnM?e>=21?$e("body").setStyles(["backgroundColor","#000","color","#fff"]):$e("body").setStyles(["backgroundColor","#fff","color","#000"]):Essence.say("You cannot use the day/night modder if it's disabled.","warn")}function Archive(t,e){return this.name=t||"Archive",this.data=e||"",this.dictionnary=[],this.updateDict=function(){for(var t,i=[],r=alphabetSort(e),s=0;s<this.data.length-1;s++)r[s]!=r[s+1]&&i.push(r[s])
...
Essence.say("Lexiq of "+this.name+": "+i+"\ncounts: "+t),this.dictionnary=occurenceSort(this.data)},this}function Machine(t,e,i,r){switch(this.capacity=i||1024,this.version=e||5,this.name=t||"Machine_"+this.version,this.version){case 1:this.base=2
break
case 2:this.base=3
break
case 3:this.base=8
...
case 6:this.base=36
break
default:this.base=16}this.operation=function(t,e,i){switch(i.normal()){case"+":return t+e
case"-":return t-e
case"*":return t*e
case"/":return t/e
case"%":return t%e
...
return Essence.say(i),JSON.parse(e)},this.unparse=function(t,e){for(var i=JSON.stringify(t),r="",s=[],n=0;n<i.length;n++)s[n]=i.charCodeAt(n),r+=2===this.base?conv(i.charCodeAt(n).toNDigits(8),10,this.base):conv(i.charCodeAt(n),10,this.base)
...
this.txt2print=""},preInit:function(){$G.t1=1e3*$G.t1.getSeconds()+$G.t1.getMilliseconds()},init:function(){$G.t2=new Date,$G.t2=1e3*$G.t2.getSeconds()+$G.t2.getMilliseconds(),$G.t=$G.t2-$G.t1>1e3?($G.t2-$G.t1)/1e3+"s":$G.t2-$G.t1+"ms",Essence.say("Page loaded in "+$G.t,"succ")},time:function(t){console.log("[%c"+getTimestamp(!0)+"%c] "+t,"color: #f00;","color: #000;")}},$G={t1:new Date,t2:0,t:null},$e=function(t){return new Element(t)}
...
for(var t=0;t<this.length;t++)Essence.say(t+": "+this[t])},Array.prototype.getOccurences=function(t){for(var e=rmDuplicates(this),i=[],r=0;r<e.length;r++)i.push(e[r]+":"+this.count(e[r])+"{"+this.positions(e[r]).toStr(!0)+"}")
if(t)for(r=0;r<i.length;r++)i[r]=parseInt(i[r].replace(/(?:.*?)\:(\d+)\{(.*?)\}/g,"$1"))
return i},Array.prototype.replace=function(t,e,i){for(var r=0;r<this.length;r++)this[r]===t&&(this[r]=e)
...
return t.mean(2)},String.prototype.normal=function(){return this.toLowerCase().remove(" ")},String.prototype.getOccurences=Array.prototype.getOccurences,String.prototype.get=function(t,e){var i=""
0>t&&!e&&(e=t,t=0),0>e&&(e=this.length+e-1)
...
return t?e.split("").join(""):e},String.prototype.replaceAll=function(t,e,i){for(var r=i?this.split(i).replace(t,e):this.replace(t,e),s=0;r.indexOf(t)>-1||s===this.length;)r=this.replace(t,e),s++
return i?r.join(i):r},Number.prototype.length=function(){if(-1!=(this+"").indexOf("."))return[parseInt((this+"").split(".")[0]).length(),parseInt((this+"").split(".")[1]).length()]
