...
return new RegExp(" "+t+" ").test(" "+this.node.className+" ")||new RegExp(" "+t+" ").test(" "+this.node[t]+" ")||this.node.style.clasName==t},this.hasCSS=function(t){if(isType(this.node,"array"))for(var e=[],i=0;i<this.node.length;i++)e.push(new RegExp(" "+t+" ").test(" "+this.node[i].style[t]+" ")||new RegExp(" "+t+" ").test(" "+this.node[i][t]+" "))
return new RegExp(" "+t+" ").test(" "+this.node.style[t]+" ")||new RegExp(" "+t+" ").test(" "+this.node[t]+" ")},this.addClass=function(t){if(isType(this.node,"array"))for(var e=0;e<this.node.length;e++)this.node[e].hasClass(clasName)||(this.node[e].className+=" "+clasName)
...
frame.resizeTo(frame.x,frame.y)}}catch(t){Essence.say("An error occurec when trying to maximise the size Because of "+t,"err")}}function getScrenDim(){return[screen.width,screen.height]}function getWinDim(){return[screen.availWidth,screen.availHeight]}function getCookie(t){for(var e,i,r=document.cookie.split(";"),s=0;s<r.length;s++)if(e=r[s].substr(0,r[s].indexOf("=")),i=r[s].substr(r[s].indexOf("=")+1),e=e.replace(/^\s + |\s + $/g,""),e===t)return unescape(i)}function setCookie(t,e,i){i%=99983489
var r=new Date
r.setDate(r.getDate()+i)
var s=escape(e)+(null===i?"":"; expires = "+r.toUTCString())
document.cookie=t+"="+s}function getMetaData(){for(var t=$n(" * meta"),e=[],i=[],r=0;r<t.length;r++)e[r]=t[r].name||t[r].httpEquiv||t[r].title,i[r]=t[r].content||t[r].value
return[e,i]}function getResources(t){for(var e=$n(" * link"),i=$n(" * script"),r=($n(" * style"),[]),s=$n(" * a"),n=$n(" * img"),o=$n(" * input image"),a=[$n(" * div"),$n(" * section"),$n(" * td"),$n(" * th"),$n(" * li")],h=0;h<e.length;h++)isNon(e[h])||(r[h]=e[h].href),Essence.say(e[h].href.split("/")[e[h].href.split("/").length-1]+" has been added to the resources getter.","info")
...
for(h=0;h<s.length;h++)isNon(s[h])||r.push(s[h].href),Essence.say(s[h].href.split("/")[s[h].href.split("/").length-1]+" has been added to the resources getter.","info")
for(h=0;h<n.length;h++)isNon(n[h])||r.push(n[h].src),Essence.say(n[h].src.split("/")[n[h].src.split("/").length-1]+" has been added to the resources getter.","info")
...
return rand(s,r,i)}function lenRand(t,e){return e?Math.floor(Math.random()*(t+1)):Math.floor(Math.random()*t)}function random16(){return(65280*Math.random()+255*Math.random())/65535}function randFloatSpread(t){return t*(.5-Math.random())}function randArr(t,e,i,r,s){for(var n=[],o=0;t>o;o++)n[o]=s?conv(rand(e||0,i||100,!r||!0),10,s||10):rand(e||0,i||100,!r||!0)
return n}function genNearlySortedArr(t,e,i){var r,s=range(t,e,i),n=[]
r=s.getIncrement(0)
for(var o=0;o<s.length;o++){var a=rand(0,r,!0)
n.push(s[o]),o>0&&0===a?swap(n,o,o-1):o>1&&a===r&&swap(n,o,o-2)}return n}function sumPow2(t,e){if(!isType(t,"Array"))return!1
for(var i=0,r=0;r<t.length;r++)i+=Math.pow(t[r],2)
return i.toNDec(e)}function conv(t,e,i,r){return r?parseFloat(t,e||2).toString(i||10):parseInt(t,e||2).toString(i||10)}function negateBin(t,e){for(var i=[],r=0,s=0;s<t.length;s++)i[s]=1-parseInt(t[s])
return r=conv(i.join("")),r++,e?conv(r,10,2).split(""):conv(r,10,2)}function min2dec(t){return 50*t/30}function dec2min(t){return 30*t/50}function toS(t,e,i){if(t||(t=e?"00:00:00.000":"00:00.000"),isType(t,"String")||(t+=""),t.length>=4&&1==t.indexOf(":"))return toS("0"+t,e,i)
if(e){var r,s,n,o
...
return i?parseFloat(a):parseFloat(a.toNDigits())}return s=t.slice(0,2),s=parseInt(s),n=t.slice(3,5),n=parseInt(n),o=t.slice(5,t.length),o=parseInt(1e3*o),o=100>o&&0!=o[0]?"0"+o:o,n+=.001*o,n+="",n.split(".")[0]=n.split(".")[0].toNDigits(),n.length>6&&(n=n.slice(0,5)||t.split(":")[1]),a=(60*s+Math.floor(1e3*n)/1e3+"").slice(0,7),i?parseFloat(a):parseFloat(a.toNDigits())}function sec2time(t,e){var i=0,r=0,s=t
return e?(s=(t%60).toNDigits(3),i=t>=3600?Math.floor(t/3600):0,r=Math.floor((t-s-3600*i)/60),s=(s+"").slice(0,6),r=0>=r?"00":r.toNDigits(),i=0>=i?"00":i.toNDigits(),0>=i?r+":"+s:i+":"+r+":"+s):(s=(t%60).toNDigits(3),r=Math.floor(t/60),s=(s+"").slice(0,6),r=0>=r?"00":r.toNDigits(),0>=r?s:r+":"+s)}function markConv(t,e,i,r){return(t/e*(i||100)).toNDec(r||2)}function nthroot(t,e,i){for(var r=t/2,s=0;60>s;s++)r+=(t-Math.pow(r,e))/(Math.pow(r+1,e)-Math.pow(r,e))
return r.toNDec(i||20)}function log(t,e){return Math.log(t)/Math.log(e||2)}function Bin(t,e,i){return C(t,i)*Math.pow(e,i)*Math.pow(1-e,t-i)}function BinCumul(t,e,i){}function Norm(t){var e=1/(1+.2316419*Math.abs(t)),i=.3989423*Math.exp(-t*t/2),r=i*e*(.3193815+e*(-.3565638+e*(1.781478+e*(-1.821256+1.330274*e))))
return r.toNDec(4)}function StdNorm(t,e,i){return Norm((i-t)/e)}function Po(t,e){return Math.exp(-t)*Math.pow(t,e)/factorial(e).toNDec(4)}function PoCumul(t,e){}function factorial(t){for(var e=t,i=1;t>i;i++)e*=i
return e}function C(t,e){return factorial(t)/(factorial(e)*factorial(t-e))}function Bin2Norm(t,e,i,r){if(t*e>5&&t*(1-e)>5){var s=">="===r?-.5:.5
return i+=s,StdNorm(t*e,Math.sqrt(t*e*(1-e)),i)}return!1}function Bin2Po(t,e,i){return t>50&&.1>e?Po(t*e,i):!1}function Po2Norm(t,e){return t>10?StdNorm(e,Math.sqrt(e)):!1}function clamp(t,e,i){return e>t?e:t>i?i:t}function clampBottom(t,e){return e>t?e:t}function clampTop(t,e){return t>e?e:t}function mapLinear(t,e,i,r,s){return r+(t-e)*(s-r)/(i-e)}function deg2Rad(t){return t*Essence.d2r}function rad2Deg(t){return t*Essence.r2d}function cels2Fahr(t){return 33.8*t}function fahr2Cels(t){return t/33.8}function primeN(t){for(var e=t.quickSort(),i=0;i<t.length;i++){t[i]%2===0&&2!=t[i]&&(e[i]="x")
for(var r=0;i>r;r++)primeCheck(e[r],e[i])&&(e[i]="x")}return e.remove("x")}function primeCheck(t,e){return t>1&&e>1&&e%t===0&&e!=t}function getClosestRoot(t,e){var i=0,r=0
t>=t/2*t/2/2-2?i=t/2:t>=t/3*t/3&&(i=t/3)
for(var s=1;e>=s;s++)for(var n=1;t>n;n++)Math.pow(n,s)===t||Math.pow(n,s-1)*n===t?r=n:(Math.pow(n,s)>t||Math.pow(n,s-1)*n>t)&&(r=n-.5)
return Math.pow(r,e)<=t?r:(r=(Math.pow(e,-2)+t/Math.pow(e,4)-t/Math.pow(e,5)+Math.pow(t,e)/Math.pow(e,Math.pow(e,3)+3)+t/Math.pow(e,2))/2,Math.pow(r,e)>t&&(r=(r+i)/2),(t/r+r)/2)}function simpleInterest(t,e,i){return t*(1+e*(i||1))}function compoundInterest(t,e,i,r){return r>1?t*Math.pow(1+e/r,(i||1)*r):t*Math.pow(1+e,i||1)}function non0(t){return 0===t?Essence.eps:t}function toFrac(t,e,i){var r=String(t),s=r.indexOf(".")
if(-1==s)return r
var n=Math.floor(t)||"",o=r.substring(s),a=e||Math.pow(10,o.length-1),h=i?Math.ceil(o*a):Math.round(o*a),l=a,c=Essence.gcd(h,l)
return l/c===1?String(n+h/c):(n&&(n+=" and "),n+String(h/c)+"/"+String(l/c))}function clearNum(t,e,i){for(var r=(Math.floor(t)+"").length/3,s="",n=0;r>n;n++)s=(t-t%Math.pow(1e3,n))%Math.pow(1e3,n+1)/Math.pow(1e3,n)+" "+s
return s=s.split(" "),s.pop(),s.join(i?",":" ")+(t%1).toNDec(e||3)}function getStep(t,e,i){return[t,e].getIncrement(i)}function quadraticSolver(t,e,i,r){var s=Math.sqrt(e,2)-4*t*i
...
break
case"yd":i=1/3456.043540706}return t*i+e}function convUnit(t,e){return fromPixel(toPixel(t),e)}function AX(t,e,i,r){var s=new ActiveXObject("Scripting.FileSystemObject")
...
s.download=e,s.innerHTML="Download File",null!=window.webkitURL?s.href=window.webkitURL.createObjectURL(r):(s.href=window.URL.createObjectURL(r),s.onclick=function(t){document.body.removeChild(t.target)},s.style.display="none",document.body.appendChild(s)),s.click()}function getFileContent(t){$G.fct=""
...
return t.getHours().toNDigits()+":"+t.getMinutes().toNDigits()+":"+t.getSeconds().toNDigits()}function getDate(t){var e=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],i=new Date
return t?i.getDate().toNDigits()+e[i.getMonth()]+i.getUTCFullYear():i.getDate().toNDigits()+"/"+i.getMonth().toNDigits()+"/"+i.getUTCFullYear()}function getTimestamp(){return getDate(!0)+"-"+getTime().replace(/\:/g,"-")}function asciiTable(t,e){"A-Z"!==t||e?"a-z"!==t||e?"A-z"!==t||e?"printable"!==t||e||(t=32,e=126):(t=65,e=122):(t=97,e=122):(t=65,e=90),t||(t=0),e||(e=255)
for(var i=[],r=t;e>=r;r++)i.push(String.fromCharCode(r))
return i}function genStr(t,e){var i,r="",s=asciiTable("a-z"),n=asciiTable("A-Z"),o=range(9),a=new Array("&","~",'"',"#","'","{","[","(","-","|","`","_","\\","^","@",")","]"," + ","=","}"," % "," * ","?",",",";",".","/",":","!"," ")
i=s.concat(n,o,a)
...
"r"===i?(s.red=255-parseInt(r[0]),s.green=r[1],s.blue=r.last()):"g"===i?(s.red=r[0],s.green=255-parseInt(r[0]),s.blue=r.last()):"b"===i?(s.red=r[0],s.green=r[1],s.blue=255-parseInt(r.last())):"y"===i?(s.red=255-parseInt(r[0]),s.green=255-parseInt(r[1]),s.blue=r.last()):"c"===i?(s.red=r[0],s.green=255-parseInt(r[0]),s.blue=255-parseInt(r.last())):"m"===i?(s.red=255-parseInt(r[0]),s.green=r[1],s.blue=255-parseInt(r.last())):"a"===i||"f"===i||"w"===i?(s.red=255-parseInt(r[0]),s.green=255-parseInt(r[1]),s.blue=255-parseInt(r.last())):(s.red=r[0],s.green=r[1],s.blue=r.last()),s.hex="#"+conv(s.red,10,16).toNDigits()+conv(s.green,10,16).toNDigits()+conv(s.blue,10,16).toNDigits(),$e(t).setCSS(e,s.hex)}function colourName2Hex(t){switch(t.normal()){case"aqua":return"#00ffff"
...
isNon(t)&&(t=63.75)
for(var s=0;257>s;s+=t)for(var n=0;257>n;n+=t)for(var o=0;257>o;o+=t)i&&Essence.say("rgb("+(e?[Math.round(s),Math.round(n),Math.round(o)].join(", "):[s,n,o].join(", "))+")"),r.push("rgb("+(e?[Math.round(s),Math.round(n),Math.round(o)].join(", "):[s,n,o].join(", "))+")")
...
return t.substring(0,t.length-2)+")"},this}function Item(t,e,i,r,s){return this.name=t||"unknown",this.category=e||"unknown",this.price=i||0,this.ageMinRequired=r||.25,this.quantity=s||1,this.firstMade=(new Date).toLocaleString(),this.dublicate=function(t,e){for(var i=0;t>i;i++)e.push(new Item(this.name,this.category,this.price,this.ageMinRequired,this.quantity))},this.remove=function(t){t.remove(this)},this.toString=function(){var t=""
for(var e in this)this.hasOwnProperty(e)&&"toString"!=e&&!isType(e,"function")&&(t+=e+"="+this[e]+", ")
return t.substring(0,t.length-2)},this}function range(t,e,i){var r=[],s=0
if(t&&!e&&!i&&0!=i)return range(0,1,t)
if(t&&e&&!i&&0!=i)return range(0,e,t)
if(t||(t=0),e||(e=1),i||(i=100),e>0)for(var n=t;i>=n;n+=e)r[s++]=n
else for(n=t;n>=i;n+=e)r[s++]=n
return r}function range2Base(t,e,i,r){var s=[],n=0
if(e>0)for(var o=t;i>=o;o+=e)s[n++]=conv(o,10,r)
else for(o=t;o>=i;o+=e)s[n++]=conv(o,10,r)
return s}function letterArray(t,e){for(var i=t.charCodeAt(0),r=e.charCodeAt(0),s=[],n="",o=i;r>=o;o++)for(var a=i;r>=a;a++)o!=a&&(n=String.fromCharCode(o)+String.fromCharCode(a),s.push(n))
return s}function mixedRange(t,e,i,r){var s=[],n=range(t,e,i)
if(r)for(;n.length>0;)s.push(n.rand()),n=n.remove(s.last())
else for(var o=t;i>=o;o++)s[o]=n.rand()
return s}function mkArray(t,e,i){var r=[]
if(1===e)if(i)for(var s=0;t>s;s++)r[s]=i
else r=new Array(t)
else if(2===e)if(i)for(s=0;t>s;s++){r[s]=new Array(t)
for(var n=0;t>n;n++)r[s][n]=i}else for(r=new Array(t),s=0;t>s;s++)r[s]=new Array(t)
else if(3===e)if(i)for(s=0;t>s;s++)for(r[s]=new Array(t),n=0;t>n;n++){r[s][n]=new Array(t)
for(var o=0;t>o;o++)r[s][n][o]=i}else for(r=new Array(t),s=0;t>s;s++)for(r[s]=new Array(t),n=0;t>n;n++)r[s][n]=new Array(t)
else{if(4!==e)throw new Error("Unvalid dimension. Only 1D-4D arrays can be made.")
if(i)for(s=0;t>s;s++)for(r[s]=new Array(t),n=0;t>n;n++)for(r[s][n]=new Array(t),o=0;t>o;o++){r[s][n][o]=new Array(t)
for(var a=0;t>a;a++)r[s][n][o][a]=i}else for(r=new Array(t),s=0;t>s;s++)for(r[s]=new Array(t),n=0;t>n;n++)for(r[s][n]=new Array(t),o=0;t>o;o++)r[s][n][o]=new Array(t)}return r}function numArr(t,e){for(var i=[],r=e||0,s=0;t>s;s++){i[s]=[]
for(var n=0;t>n;n++)i[s][n]=r++}return i}function swap(t,e,i){var r
...
return i[0]=["i","j","k"],i[1]=[t.x,t.y,t.z],i.last([e.x,e.y,e.z]),r[0]=t.y*e.z-t.z*e.y,r[1]=t.x*e.z-t.z*e.x,r.last(t.x*e.y-t.y*e.x),r[0]+"i + "+r[1]+"j + "+r.last()+"k"}function vector2Point(t){return new Pt(t.x,t.y,t.z)}function vector2PointForm(t){return"("+t.split("i")[0].clean()+", "+t.split("i")[1].split("j")[0].slice(1,t.split("i")[1].split("j")[0].length).clean()+", "+t.split("i")[1].split("j")[1].split("k")[0].clean()+")"}function scalarProd(t,e){return t.x*e.x+t.y*e.y+t.z*e.z}function union(t,e,i,r){return r?rmDuplicates(t.concat(e,i)).quickSort():rmDuplicates(t.concat(e,i))}function intersection(t,e,i,r){var s=[]
t=t.quickSort(),e=e.quickSort(),i=i?i.quickSort():!1
for(var n in t)e.indexOf(t[n])>-1&&isNon(i)?s.push(t[n]):e.indexOf(t[n])>-1&&i.indexOf(t[n])>-1&&s.push(t[n])
return r?rmDuplicates(s).quickSort():rmDuplicates(s)}function complement(t,e,i,r){var s=[]
t=t.quickSort(),e=e.quickSort(),i=i?i.quickSort():!1
for(var n in t)-1===e.indexOf(t[n])&&isNon(i)?s.push(t[n]):-1===e.indexOf(t[n])&&-1===i.indexOf(t[n])&&s.push(t[n])
return r?rmDuplicates(s).quickSort():rmDuplicates(s)}function symDif(t,e,i,r){var s=[]
t=t.quickSort(),e=e.quickSort(),i=i?i.quickSortort():!1
for(var n in t)-1===e.indexOf(t[n])&&isNon(i)?s.push(t[n]):-1===e.indexOf(t[n])&&-1===i.indexOf(t[n])&&s.push(t[n])
for(n in e)-1===t.indexOf(e[n])&&isNon(i)?s.push(e[n]):-1===t.indexOf(e[n])&&-1===i.indexOf(e[n])&&s.push(e[n])
if(i)for(n in i)-1===t.indexOf(i[n])&&-1===e.indexOf(i[n])&&s.push(i[n])
return r?rmDuplicates(s).quickSort():rmDuplicates(s)}function bitStr(t,e){var i=[]
for(var r in t)i[r]=t[r]===e[r]?1:0
...
var s=r?new Array(t.length):t[0]&&e[0]
for(var n in t)r?s[n]=t[n]&&e[n]:s=s&&t[n]&&e[n]
return s}function Or(t,e,i,r){toSameLength(t,e,i||null)
var s=r?new Array(t.length):t[0]||e[0]
for(var n in t)r?s[n]=t[n]||e[n]:s=s||t[n]||e[n]
return s}function Xor(t,e,i,r){toSameLength(t,e,i||null)
var s=r?new Array(t.length):xor(t[0],e[0])
for(var n in t)r?s[n]=xor(t[n],e[n]):s=xor(s,xor(t[n],e[n]))
return s}function Imply(t,e,i,r){toSameLength(t,e,i||null)
var s=r?new Array(t.length):!t[0]||e[0]
for(var n in t)r?s[n]=!t[n]||e[n]:s=!s||!t[n]||e[n]
return s}function manhattanDist(t,e){return Math.abs(Math.abs(t[0]-e[0])+Math.abs(t[1]-e[1]))}function euclidianDist(t,e){return Math.sqrt(Math.pow(t[0]-e[0],2)+Math.pow(t[1]-e[1],2))}function diagDist(t,e){return Math.max(Math.abs(t[0]-e[0]),Math.abs(t[1]-e[1]))}function h(t,e,i){for(var r=new Array(t.length),s=0;s<t.length;s++){r[s]=new Array(t[s].length)
for(var n=0;n<t[s].length;n++)r[s][n]=i([s,n],lookfor(t[s][n],e))||euclidianDist([s,n],lookfor(t[s][n],e))}return Essence.say(console.table(r)),r.sum2d()}function lookfor(t,e,i){for(var r=0;r<e.length;r++)for(var s=0;s<e[r].length;s++)if(e[r][s]===t)return i?[s,r]:[r,s]
...
i+=r+t
try{r.left&&e.unshift(r.left),r.right&&e.unshift(r.right)}catch(s){Essence.say(s+" caused "+this+".printBFS("+t+") to go wrong","err")}}return t},this.toString=function(){var t="TreeNode(payload = "+this.payload+", "
...
s.left&&r.unshift(s.left),s.right&&r.unshift(s.right),e++,i++}return[-1,i]},this.sum=function(){var t=this.payload
return this.left&&(t+=this.left.sum()),this.right&&(t+=this.right.sum()),t},this.min=function(){var t=this.payload
return this.left&&(t=Math.min(t,this.left.min())),this.right&&(t=Math.min(t,this.right.min())),t},this.max=function(){var t=this.payload
...
return this.top=this.top.next,e}for(var i=0;t>i;i++)this.pop()},this.isEmpty=function(){return null==this.top},this.size=function(t){return null!=this.top?this.size(t+1):t}}function Queue(t,e){this.value=isType(e,"Number")?new Array(e):[],this.limit=e||null,t&&this.value.push(t),this.enqueue=function(t){if(this.isFull())throw new Error("Queue overflow !")
...
return t},this.equals=function(t){return this.toString()===t.toString()},this.remove=function(t){for(var e=this.front;null!=e;)null==e.payload||e.payload!=t&&!e.payload.equals(t)||(e=null),e=e.next},this.insertAt=function(t,e){this.back.next=new Node(e),0===t?this.front=new Node(e):t===this.len?this.back=new Node(e):t>this.len&&this.enqueue(e)}}function Astar(){for(var t,e,i,r,s,n=[e],o=[];n!=[];){var a=[],l=n[0].f
for(var c in n)a.push(n[c].f),l=Math.min(l,n[c].f)
if(i=n.filter(function(t){return t.f===l?t:void 0})[0],n=n.filter(function(t){return t.f!=l?t:void 0}),i===t){for(r in i)if(i.hasOwnProperty(nodeSuccessor)){nodeSuccessor.f=i.f+h(nodeSuccessor,i)
var u=lookfor(nodeSuccessor,n)
if(-1!=u){if(u=u[0],i.f<n[u]||-1!=lookfor(nodeSuccessor,o)&&i.f<n[lookfor(nodeSuccessor,o)][0])continue
n=n.remove(nodeSuccessor),o=o.remove(nodeSuccessor),nodeSuccessor.parent=i,s=h(nodeSuccessor,t),n.push(nodeSuccessor)}o.push(i)}throw"Solution found !"}}}function IDAstar(){}function readCoord(t,e){var i=t.slice(1,t.length-1).split(", ")
...
for(a+="</tr>",h=0;h<i.length;h++){if(a+=e?"<tr><th>"+e[h]+"</th>":"<tr>",n)for(var l=0;l<i[h].length;l++)a+="<td>"+i[h][l]+"</td>"
else a+="<td>"+i[h]+"</td></tr>"
a+="</tr>"}return a+="</table><style > table{background: #000;}table, td, th {border: 1px solid #000; color: #000; background: #fff;} tr:nth-child(even) td{background: #ddd;} tr td:hover{background: #bbb;}</style>"}function colourTable(t,e,i,r,s,n){var o=t?"<table id = "+r+" style = "+n+" cellspacing = 0 cellpadding = 2><caption>"+t+"</caption>":"<table>"
if(e){o+="<tr>"
for(var a=0;a<e.length;a++)o+="<th>"+e[a]+"</th>"
o+="</tr>"}for(a=0;a<i.length;a++){if(o+="<tr>",s)for(var h=0;h<i[a].length;h++)o+=isValid(i[a][h],"color")?"<td style = 'background:"+i[a][h]+";'><br /></td>":"<td>"+i[a][h]+"</td>"
else o+="<td style = 'background:"+i[a]+";'><br /></td>"
o+="</tr>"}return o+="</table><style > table{background: #000;}table, td, th {border: 1px solid #000; color: #000; background: #fff;} tr:nth-child(even) td{background: #ddd;} tr td:hover{background: #bbb;}</style>"}function database(t,e,i,r,s,n){this.name=t||"Database",this.headerRow=isNon(e)?range(0,1,100):e,this.content=isNon(i)?[].fill("..."):i,this.headerCol=r||["Index","Value"],this.admin=s||"Anonymous",this.version=n||1,this.val=new Array(this.headRow.length)
...
return String.fromCharCode(i+e)}function encrypt(t,e){if(!e){var i=t.length,r=0,s=Math.floor(i/2)
s=i%2===0?t.charCodeAt(s):(t.charCodeAt(t[s-1])+t.charCodeAt(t[s]))/2,r=s>=97&&122>=s?2:s>=65&&90>=s?1:s-2*Math.floor(s/2)===0?-1:2,e=Math.round((Math.pow(2,7)+t.sum()-48)/t.prod())+r}for(var n="",o=0;o<t.length;o++)n+=trans(t[o],e)
return i=s=r=void 0,n}function decrypt(t,e){var i=""
if(e)for(var r=1;r<=t.length;r++)i+=trans(t[r-1],e)
else for(i=new Array(131073),r=-65536;65537>r;r++){i[r+65536]=""
for(var s=0;s<t.length;s++)i[r+65536]+=trans(t[s],r%65537)}return console.log(console.table(i)),e?t+" => "+i:complexTable("Decryption result for <i>"+t+"</i>",range(-65536,1,65536),i,["Key","Result"],"decrypted_"+t,!1,Essence.css)}function abcEncode(t){var e=new Array(t.length)
if(isType(t,"string")||isType(t,"array")){for(var i in t)switch(t[i].toLowerCase()){case" ":e[i]="00"
break
case"a":e[i]="01"
break
case"b":e[i]="02"
...
break}this.slots[e]!=t&&this.nb_slots<this.maxsize&&(this.nb_slots+=this.maxsize/this.nb_slots,this.slots[this.nb_slots]=JSON.stringify(this))},this.rm=function(t){this.slots[t]=null},this.store=function(){localStorage["server_"+this.name]=JSON.stringify(this)},this.update=function(){localStorage["server_"+this.name]?(this.name=localStorage["server_"+this.name].name,this.admin=localStorage["server_"+this.name].admin,this.version=localStorage["server_"+this.name].version,this.maxsize=localStorage["server_"+this.name].maxsize,this.nb_slots=localStorage["server_"+this.name].nb_slots,this.slots=JSON.parse(localStorage["server_"+this.name].slots),this.data=localStorage["server_"+this.name].data):this.store()},this.fire=function(t){for(var e in this.slots)this.slots[e][0]==t.name&&this.slots[e][1]==t.author&&this.rm(e)},this.reset=function(){for(var t in this.slots)this.rm(t)},Essence.addServer(this)}function CECheck(t,e,i,r,s){window.defaultStatus="Evalue the connexion and see the downloading speed",e||(e="img/random2000x2000.jpg"),i||(i=61784),r||(r=100),s||(s=2e4)
var n=new Image
$G.t1=(new Date).getTime(),$G.t2=0,n.src=e+"?t1="+$G.t1
var o=0
$e("#"+slt).write("Verification in progress..."),setTimeout("CETimer("+t+", "+n+", "+o+", "+r+", "+s+", "+i+")",r)}function CETimer(t,e,i,r,s,n){i++,$e("#"+t).write("Verification in progress..."),i*r>=s?$e("#"+t).write(evalDownload(0)):e.complete?($G.t2=(new Date).getTime(),$e("#"+t).write(evalDownload(n/($G.t2-$G.t1))),console.log("["+Date()+"] Connexion: "+(n/($G.t2-$G.t1).toNDigits(3)+" kbps"))):setTimeout("CETimer("+t+", "+e+", "+i+", "+r+", "+s+", "+sz+")",r)}function evalDownload(t){var e=""
...
return e=t>0&&30>=t?"Excellent":t>30&&60>=t?"Really good":t>60&&100>=t?"Good":t>100&&200>=t?"Okay":t>200&&300>=t?"Poor":t>300&&400>=t?"Bad":"Really bad",e+" ("+t+" ms)"}function msgBox(t,e,i,r,s,n){t=t.toLowerCase().remove(" ")
...
return i.length<20&&(i+=t[randTo(t.length-1)]),i}function genHash(t){for(var e,i,r="",s=(821-t.sum())/t.prod()*t.charCodeAt(0).toNDigits(1),n=0;n<t.length;n++)e=t.charCodeAt(n)+s.toNDigits(1)%255,i=Math.abs(t.charCodeAt(n)+s).toNDigits(1),32>i&&(i+=48),r+=String.fromCharCode(clamp(i%127,32,126))
return r}function weekDay(t){var e,i=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],r=parseInt(t.split("/")[0]),s=parseInt(t.split("/")[1]),n=parseInt(t.split("/").last())
return e=Math.floor(n/100),n%=100,i[Math.round((1+2.6*s-.2+r+n+n/4+e/4-2*e)%7)-1]}function dayOfWeek(t){var e,i=parseInt(t.split("/")[0]),r=[0,3,3,6,1,4,6,2,5,0,3,5],s=["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],n=parseInt(t.split("/").last())%100+Math.floor(t.split("/").last()/4),o=Math.floor(t.split("/").last()/100%4)
...
i=parseInt(i)||10
for(var n=[],o=(s-r)/(i-1),a=0;i>a;a++)n.push(conv(r+a*o,10,16))
return n}function radialGradient(t,e,i){}function parseURL(t,e){var i=location.href.split("?")
if(i.length>1){var r=i[1].split("&")
if(isType(t,"Array"))for(var s=0;s<t.length;s++)for(var n=0;n<r.length;n++){var o=r[n].split("=")
o[0]===t[s]&&(isType(e,"Array")?e[s](o[1],o[0]):e(o[1],o[0]))}else for(n=0;n<r.length;n++)o=r[n].split("="),o[0]===t&&e(o[1])}}function GET(t){return t===new RegExp("[?&]"+encodeURIComponent(t)+"=([^&] * )").exec(location.search)?decodeURIComponent(t[1]):void 0}function POST(t,e){var i=document.createElement("form")
...
for(var s=0;s<e.maxLength();s++){for(var n=0;n<e.length-r;n++)0==s&&e[n].charAt(s)>e[n+1].charAt(s)?(swap(e,n,n+1),i=!0):e[n].charAt(s-1)===e[n+1].charAt(s-1)&&e[n].charAt(s)>e[n+1].charAt(s)&&(swap(e,n,n+1),i=!0)
r++}}return e.trimAll("r")}function timeUp(t,e,i){var r=new Date,s=0
...
t&&($G.dnM=!$G.dnM),$G.dnM?e>=21?$e("body").setStyles(["backgroundColor","#000","color","#fff"]):$e("body").setStyles(["backgroundColor","#fff","color","#000"]):Essence.say("You cannot use the day/night modder if it's disabled.","warn")}function archive(t,e){return this.name=t,this.data=e,this.dictionnary=[],this.updateDict=function(){for(var t,i=[],r=alphabetSort(e),s=0;s<e.length-1;s++)r[s]!=r[s+1]&&i.push(r[s])
for(i=rmDuplicates(i),console.log(i.getOccurences()),t=new Array(i.length),r=[],s=0;s<i.length;s++)t[s]=e.count(i[s]),Essence.say("lexiq["+s+"]="+i[s]+" is present "+timesLiteral(t[s]),"inf"),r[s]=i[s]+t[s]
Essence.say("Lexiq of "+this.name+": "+i+"\ncounts: "+t)},this}function Machine(t,e,i,r){switch(this.capacity=i||1024,this.version=e||5,this.name=t||"Machine_"+this.version,this.version){case 1:this.base=2
break
case 2:this.base=3
break
case 3:this.base=8
...
return t},this.memory=new Memory(this.capacity,r||"",this.name),this.send=function(t,e){POST(e,"msg = "+this.parse(t))},this.parse=function(t){for(var e="",i=[],r=0;r<t.length;r++)i[r]=conv(t[r],this.base),e+=String.fromCharCode(conv(t[r],this.base))
return console.log(i),JSON.parse(e)},this.unparse=function(t){for(var e=JSON.stringify(t),i="",r=[],s=0;s<e.length;s++)r[s]=e.charCodeAt(s),i+=2===this.base?conv(e.charCodeAt(s).toNDigits(8),10,this.base):conv(e.charCodeAt(s),10,this.base)
return Essence.say(t+"= "+r,"info"),2===this.base?i.divide(8):i.divide(2)},this.store=function(t){this.memory.add(this.unparse(t)),this.memory.save()},this.show=function(){for(var t=0;t<this.memory.slots.length;t++)Essence.say(this.memory.slots[t])},this.specs=function(){return"Name: "+this.name+"\nCapacity: "+this.capacity+" bits\nMemory: \n"+this.memory.toString()}}function Memory(t,e,i){this.capacity=t||1024,this.slots=new Array(this.capacity),this.type="local"==e?"local":"session",this.name=i?i+"_"+this.type+"M"+log(this.capacity):this.type+"M"+log(this.capacity),this.free=0,this.save=function(){for(var t=0;t<this.slots.length;t++)"local"===this.type?localStorage.setItem(this.name+"#"+t,this.slots[t]):sessionStorage.setItem(this.name+"#"+t,this.slots[t])},this.remove=function(t){this.slots.remove(JSON.stringify(t)),"local"===this.type?localStorage.setItem(this.getLocation(t),void 0):sessionStorage.setItem(this.getLocation(t),void 0)},this.getLocation=function(t){if("local"===this.type){for(var e in localStorage)if(localStorage[e]===JSON.stringify(t))return e}else for(e in sessionStorage)if(sessionStorage[e]===JSON.stringify(t))return e
...
this.template+="</tr>"}return this.template+="</table>",this.template},this.genPage=function(){}}function WebApp(t,e,i,r,s){this.name=t||"Web App",this.path=e||"",this.version=r||1,this.author=i||"Maximilian Berkmann",this.dirs=["img","script","style"],this.pages=[new WebPage(this.name,"index",this.path,this.author,this.ver,this.stct),new WebPage("Contact us","contact",this.path,this.author,this.ver,this.stct),new WebPage("About us","about",this.path,this.author,this.ver,this.stct)],this.build=function(){for(var t=0;t<this.pages.length;t++)this.pages[t].genPage()}}function virtualHistory(t){this.src=t,this.DEFAULT_STATE=t,this.states=new Set(this.src),this.state=0,this.reset=function(){this.src=this.DEFAULT_STATE},this.update=function(t){this.src!=t&&this.add(t)},this.add=function(t){if(isType(t,"array"))for(var e=0;e<t.length;e++)this.add(t[e])
...
return e},Object.prototype.isIterable=function(){return isType(this,"String")||isType(this,"Array")||isType(this,"Object")},Object.prototype["delete"]=function(){this.property_=null},Object.prototype.equals=function(t){return this.toString()===t.toString()||this.toLocaleString()===t.toLocaleString()},Array.prototype.first=function(t){return t?this[0]=t:this[0]},Array.prototype.last=function(t){return t?this[this.length-1]=t:this[this.length-1]},Array.prototype.lastIndex=function(){return this.length-1},Array.prototype.midIndex=function(t){return t?Math.floor(this.length/2)-1:Math.floor(this.length/2)},Array.prototype.even=function(){for(var t=[],e=0;e<this.length;e+=2)t.push(this[e])
...
if(!e)return!1
for(var i=1;i<this.length;i++){if(!e)return!1
e=e&&t(this[i])}return e},Array.prototype.remove=function(t){var e=this
...
e=r}return e},Array.prototype.debug=function(){Essence.say("Debugging the following array: "+this)
for(var t=0;t<this.length;t++)Essence.say(t+": "+this[t])},Array.prototype.getOccurences=function(){for(var t=rmDuplicates(this),e=[],i=0;i<t.length;i++)e.push(t[i]+":"+this.count(t[i])+"["+this.positions(t[i]).toStr(!0)+"]"),Essence.say(t[i]+": "+this.positions(t[i]).toStr(!0))
return e},Array.prototype.replace=function(t,e,i){for(var r=0;r<this.length;r++)this[r]===t&&(this[r]=e)
...
if(!t&&!e||0===t&&e>=this.length-1)for(var r=0;r<this.length;r++)for(var s=0;s<this[r].length;s++)i+=this[r][s]
else if(t&&!e)for(r=t[0];r<this.length;r++)for(s=t[1];s<this[r].length;s++)i+=this[r][s]
else for(r=t[0];r<e[0];r++)for(s=t[1];s<e[1];s++)i+=this[r][s]
...
for(i=t;s>=i;){for(;this[i]>r;)i++
for(;this[s]<r;)s--
s>=i&&(swap(this,i,s),i++,s--)}t>i-1&&this.revSort(t,i-1),i>e&&this.revSort(i,e)}return this},Array.prototype.bubbleSort=function(t){var e=this,i=1,r=!0
if(isNon(t)||isType(t,"string")&&"a"===t[0].toLowerCase())for(;r;){r=!1
for(var s=0;s<=e.length-i;s++)e[s]>e[s+1]&&(e=swap(e,s,s+1),r=!0),s<e.length-(i+1)&&(e[s]>e[s+2]&&(e=swap(e,s,s+2)),e[s+1]>e[s+2]&&(e=swap(e,s+1,s+2),r=!0)),s<e.length-(i+2)&&(e[s]>e[s+3]&&(e=swap(e,s,s+3)),e[s+1]>e[s+3]&&(e=swap(e,s+1,s+3)))
i++}else if(1==t||isType(t,"string")&&"d"===t[0].toLowerCase())for(;r;){for(r=!1,s=0;s<=e.length-i;s++)e[s]<e[s+1]&&(e=swap(e,s,s+1),r=!0),s<e.length-(i+1)&&(e[s]<e[s+2]&&(e=swap(e,s,s+2)),e[s+1]<e[s+2]&&(e=swap(e,s+1,s+2),r=!0)),s<e.length-(i+2)&&(e[s]<e[s+3]&&(e=swap(e,s,s+3)),e[s+1]<e[s+3]&&(e=swap(e,s+1,s+3)))
...
var s=this[t]
this[t]=e,this[i]=s}return this},Array.prototype.maxSort=function(){var t=this.min(),e=this.median(),i=this.max(),r=new Array(this.length),s=this.getIncrement(3),n=this.quartile(1),o=this.quartile(3)
r[0]=t,r[r.length-1]=i,r.length%2!=0&&(r[r.length/2]=e,r[r.length/4]=n,r[3*r.length/4]=o)
for(var a=1;a<this.length-1;a++)this[a]===Math.floor(r[0]+a*s)?r[a]=this[a]:this[a]===Math.round(r[0]+a*s)?r[a]=this[a]:this[a]==Math.ceil(r[0]+a*s)?r[a]=this[a]:this[a]>=Math.floor(r[0]+a*s)&&this[a]<=Math.ceil(r[0]+a*s)&&(r[a]=this[a])
for(a=this.length-1;a>1;a--)this[a]===Math.floor(r[r.length-1]-a*s)&&isNon(r[a])?r[a]=this[a]:this[a]===Math.round(r[r.length-1]-a*s)&&isNon(r[a])?r[a]=this[a]:this[a]===Math.ceil(r[r.length-1]+a*s)&&isNon(r[a])?r[a]=this[a]:this[a]>=Math.floor(r[r.length-1]+a*s)&&this[a]<=Math.ceil(r[0]+a*s)&&isNon(r[a])&&(r[a]=this[a])
for(a=1;a<this.length-1;a++)for(var h=0;h<this.length;h++)this[h]===Math.floor(r[0]+a*s)?r[a]=this[h]:this[h]===Math.round(r[0]+a*s)?r[a]=this[h]:this[h]===Math.ceil(r[0]+a*s)?r[a]=this[h]:this[h]>=Math.floor(r[0]+a*s)&&this[h]<=Math.ceil(r[0]+a*s)&&(r[a]=this[h])
return r},Array.prototype.cenSort=function(t,e){var i=new Array(this.length)
if(t||e||(t=Math.floor(this.length/2),e=Math.ceil(this.length/2)),!(this.length<=1)){for(var r=this[Math.floor((e+t)/2)],s=e,n=t;s>=n;){for(;this[n]<r;)n--
for(;this[s]>r;)s++
...
t=e=i},Array.prototype.setSort=function(){for(var t=[],e=this.length,i=[],r=0;1e3>r;r++)t[r]=0
...
for(var r=this.get(e,i),s=t.get(e,i),n=0;n<r.length;n++)r[n]=s[n]
return r},Array.prototype.exchange=function(t,e,i){e=e||0,i=i||this.length-1
for(var r=this.get(e,i),s=t.get(e,i),n=0;n<r.length;n++){var o=r[n]
r[n]=s[n],s[n]=o}return r},Array.prototype.rot=function(t){var e
...
90===t?(e=this[0].get(-1),this[0][0]=this.last()[0],this.last()[0]=this.last().last(),this.last().last(this[0].last()),this[0].last(e[0]),this[0][1]=this[1][0],this[1][0]=this.last()[1],this.last()[1]=this[1].last(),this[1].last(e[1])):-90===t?(e=[this[0][0],this[0][1]],this[0][0]=this[0].last(),this[0].last(this.last().last()),this.last().last(this.last()[0]),this.last()[0]=e[0],this[0][1]=this[1].last(),this[1].last(this.last()[1]),this.last()[1]=this[1][0],this[1][0]=e[1]):180===Math.abs(t)&&(e=[this[0][0],this[0][1],this[0].last(),this[1][0]],this[0][0]=this.last().last(),this.last().last(e[0]),this[0].last(this.last()[0]),this.last()[0]=e.last(),this[0][1]=this.last()[1],this.last()[1]=e[1],this[1][0]=this[1].last(),this[1].last(e[3]))}return this},Array.prototype.numElm=function(){return this.toString().split(",").length},Array.prototype.size=function(t){return t?this.length+"x"+this.maxLength():[this.length,this.maxLength()]},Array.prototype.det=function(){var t=0
...
this[t][e]=this[e][t],this[e][t]=i}if("4x4"===this.size(!0)){var r=this[3].last()
this[3].last(this.last()[3]),this.last()[3]=r}return this},Array.prototype.lookFor=function(t){for(var e=0;e<this.length;e++)if(this[e]===t)return e
return-1},Array.prototype.divide=function(t){for(var e=new Array(Math.round(this.length/t)).fill(""),i=0,r=0;r<e.length;r++)for(var s=0;t>s;s++)e[r]+=this[i++]
return e},Array.prototype.getAdjoint=function(){var t=this.translate(),e=mkArray(this.length,2,Essence.eps)
return 4===t.numElm()&&2===t.length?(e[0]=[t[1][1],-t[1][0]],e[1]=[-t[0][1],t[0][1]]):9===t.numElm()&&3===t.length?(e[0]=[t[1][1]*t.last().last()-t[1].last()*t.last()[1],-(t[1][0]*t.last().last()-t[1].last()*t.last()[0]),t[1][0]*t.last()[1]-t[1][1]*t.last()[0]],e[1]=[-(t[0][1]*t[1].last()-t[0].last()*t[1][1]),t[0][0]*t.last().last()-t[0].last()*t.last()[0],-(t[0][0]*t.last()[1]-t[0][1]*t.last()[0])],e.last([t[0][1]*t[1].last()-t[0].last()*t[1][1],-(t[0][0]*t[1].last()-t[0].last()*t[1][0]),t[0][0]*t[1][1]-t[0][1]*t[1][0]])):Essence.say("Unsupported matrix format","error"),e},Array.prototype.dotProd=function(t){for(var e=[],i=0;i<this.length;i++){e[i]=[]
...
for(var s=0;s<this[r].length;s++)i[r][s]="a-b"===e?t-this[r][s]:this[r][s]-t}return i},Array.prototype.dotFrac=function(t,e){var i=[]
e=e.toLowerCase().remove(" ")
for(var r=0;r<this.length;r++){i[r]=[]
for(var s=0;s<this[r].length;s++)i[r][s]="a/b"===e?t/this[r][s]:this[r][s]/t}return i},Array.prototype.toStr=function(t){var e=""
...
return t},Array.prototype.inv=function(){return this.dotProd(1/this.det(),this.getAdjoint())},Array.prototype.mix=function(){for(var t=mixedRange(0,1,this.length-1),e=[],i=0;i<this.length;i++)e[i]=this[t[i]]
return e},Array.prototype.append=function(t){for(var e=0;e<t.length;e++)this.push(t[e])},Array.prototype.preppend=function(t){for(var e=0;e<t.length;e++)this.unshift(t[e])},Array.prototype.unique=function(){for(var t=[],e=0;e<this.length;e++)1===this.count(this[e])&&t.push(this[e])
return t},Array.prototype.to1d=function(t){for(var e=this,i=0;i<e.length;i++)e[i]=e[i].join(t||"")
return e},Array.prototype.uniform=function(t){for(var e=this,i=e.maxLength(),r=0;r<e.length;r++)for(;e[r].length<i;)isType(e[r],"Array")?e[r].push(t||" "):e[r]+=t||" "
...
else if(this.indexOf(t)>-1)throw"the object "+t.toString()+"is already present in "+this.toString()},Array.prototype.replaceAll=function(t,e){for(var i=this.replace(t,e);i.indexOf(t)>-1;)i=this.replace(t,e)
return i},String.prototype.remove=function(t){var e=this
...
for(var i=mixedRange(0,1,this.length-1),r=this.split(seperator),s=[],n=0;n<this.length;n++)s[n]=r[i[n]]
return s.join(e)},String.prototype.divide=function(t){for(var e=new Array(Math.round(this.length/t)).fill(""),i=0,r=0;r<e.length;r++)for(var s=0;t>s;s++)e[r]+=this[i++]
...
for(var i in t)t.hasOwnProperty(i)&&(e.push(t[i]+":"+this.count(t[i])+"{"+this.positions(t[i]).toStr(!0)+"}"),Essence.say(t[i]+": "+this.positions(t[i]).toStr(!0)))
return e},String.prototype.chunk=function(t,e){return 0>e&&(e=this.split(" ").length+e-1),this.split(" ").get(parseInt(t),parseInt(e)).join(" ")},String.prototype.get=function(t,e){var i=""
0>t&&!e&&(e=t,t=0),0>e&&(e=this.length+e-1)
...
return t?e.split("").join(""):e},String.prototype.replaceAll=function(t,e){for(var i=this.replace(t,e);i.indexOf(t)>-1;)i=this.replace(t,e)
return i},Number.prototype.length=function(){if(-1!=(this+"").indexOf("."))return[parseInt((this+"").split(".")[0]).length(),parseInt((this+"").split(".")[1]).length()]
...
break
case 2:r=s(t,i)<<18|s(t,i+1)<<12,o.push(String.fromCharCode(r>>16))}return o.join("")},getbyte:function(t,e){var i=t.charCodeAt(e||0)
...
var e,i,r=this.PADCHAR,s=this.ALPHA,n=this.gb,o=[]
t+=""
