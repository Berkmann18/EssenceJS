...
Array.prototype.max = function (start, end) { //get the maximum value of the array
	...
	else if(start && !end) for(var i = start+1; i < this.length; i++) m = Math.max(m, this[i]);
	else for(var i = start+1; i <= end; i++) m = Math.max(m, this[i]);
	return m
}
...
Array.prototype.min = function (start, end) { //get the minimum value of the array
	...
	else if(start && !end) for(var i = start+1; i < this.length; i++) m = Math.min(m, this[i]);
	else for(var i = start+1; i <= end; i++) m = Math.min(m, this[i]);
	return m
}
...
Array.prototype.all = function (callback) { //check if all items of the array applies to the conditions specified by the callback function
	...
	for(var i = 1; i < this.length; i++) bool = bool && callback(this[i]);
	return bool
}
...
Array.prototype.remove = function (c) { //remove c from the array without affecting the initial array
	...
	var str = this;
	if (isArray(c)) {
		for(var i = 0; i < c.length; i++) str = str.remove(c[i]);
	}else{
...
Array.prototype.sum = function (start, end) { //the sum of every terms of the array
	...
	else if(start && !end) for(var i = start; i < this.length; i++) s += this[i];
	else for(var i = start; i <= end; i++) s += this[i];
	return s
}
...
Array.prototype.sum2d = function (start, end) { //sum for 2D arrays where start = [i, j] and end [i, j]
	...
	if ((!start && !end) || (start == 0 && end >= this.length-1)) {
		...
		for (var i = start[0]; i < this.length; i++) {
			for(var j = start[1]; j < this[i].length; j++) s += this[i][j];
		}
	}else{
		for (var i = start[0]; i < end[0];i++) {
			for(var j = start[1]; j < end[1]; j++) s += this[i][j];
		}
	}
	return s
}
...
Array.prototype.maxSort = function () { //My own sorting algorithm
	...
	for (var i = this.length-1; i > 1; i--) { //same thing but from the end to complete the missing ones
...
Array.prototype.rot = function (deg) { //rotate a matrix by n%90 degrees. Useful for Rubik's cubes simulator and other matrix based simulations/calculations!
	...
	if (this.numElm() == 4 && this.length == 2) { //2x2 matrix
		...
		if (deg == 90) {
			tmp = this[0].get(0, -1);//get all but the last element of the first row
...
Array.prototype.uniform = function (cr) { //ensure that all elements in the array are of the same length
	...
	console.log(ml);
	for (var i = 0; i < res.length; i++) {
		// if(res[i].length === ml) continue; //saves time and cost
...
Array.prototype.unzip = function () {
	var res = [], j;
	for (var i = 0; i < this.length; i++) {
		if(/\s(?:xx)(\d+)/g.test(this[i])) res.push(this[i].get(0, this[i].indexOf(" xx")).trim().repeat(this[i][this[i].indexOf(" xx")+3]));
		else res.push(this[i]);
		console.log("#"+i);
	}
	return res
...
String.prototype.unzip = function () {
	...
	for (var i = 0; i < this.length-1; i++) {
		if(/\s(?:xx)(\d+)/g.test(this[i])) res.push(this[i].get(0, this[i].indexOf("xx")-2).repeat(this[i][this[i].indexOf("xx")+2]));
		else res.push(this[i]);
	}
	return res
};
...
function getResources (rmEmpty) { //get all the resources of a page apart from the in-CSS ones
	...
	for (var i = 0; i < hypertxt.length; i++) {
...
function mixedRange (min, inc, max, noRepeat) { //like randArr but with unique values and using a Fisher Yates-like approach
	...
	if (noRepeat) {
		...
		for(var i = min; i <= max; i++) val[i] = available[randTo(available.length-1)];
	}
	return val
}
...
function mkArray (len, dim, fill) { //make an array of len^dim
	...
	if (dim == 1) {
		...
		if (!fill) {
			arr = new Array(len);
			for(var i = 0; i < len; i++) arr[i] = new Array(len);
		}else{
			for (var i = 0; i < len; i++) {
				arr[i] = new Array(len);
		...
		if (!fill) {
			...
			for (var i = 0; i < len; i++) {
				arr[i] = new Array(len);
				for(var j = 0; j < len; j++) arr[i][j] = new Array(len);
			}
		}else{
			for (var i = 0; i < len; i++) {
				arr[i] = new Array(len);
		...
		if (!fill) {
			...
			for (var i = 0; i < len; i++) {
				...
				for (var j = 0; j < len; j++) {
					arr[i][j] = new Array(len);
					for(var k = 0; k < len; k++) arr[i][j][k] = new Array(len);
				}
			}
		}else{
			for (var i = 0; i < len; i++) {
...
function toSameLength (a, b, cr) { //make sure that a and b are of the same lengths and fill the empty spaces with cr
	...
	if (a.length > b.length) {
		...
		for(var i = a.length; i < b.length; i++) isType(a[i], "String")? a += cr: a.push(cr);
	}
	return [a, b]
}
...
function rmConsecDuplicates (arr) { //remove the consecutive duplicated values in an array
	...
	j = 0;
	for (var i = 0; i < arr.length-1; i++) { //double enforced check
...
function database (name, headR, cells, headC, admin, ver) { //local database
	...
	for(var i = 0; i < this.headerCol.length; i++) this.val[0][i] = this.headerCol[i];
...
var base64 = {
	...
	getbyte64: function (s, i) {
		...
		var i = this.ALPHA.indexOf(s.charAt(i || 0));
...
function alphabetSort (x, k) { //sort alphabetically the elements in x
	...
	while (s) {
		...
		for (var i = 0; i < res.length-j; i++) {
			Essence.say("Reading #"+i+" "+res[i]);
			if (res[i].charAt(0)>res[i+1].charAt(0)) {
				swap(res, i, i+1);
				s = true;
			}
		}
...
function Template (name, path, txt, params) { //JavaScript templating + conversion
	...
	this.gen = function (obj) { //generate a text/code from the template using the keys of the object
		...
		for(var i = 0; i < this.special.length; i++) res = res.replace("%"+this.special[i]+"%", this.specialEq[i]);
		return res
	}
