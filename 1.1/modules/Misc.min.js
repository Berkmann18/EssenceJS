...
return n?c():(a.map(function(r){h.push(setTimeout(function(){console.clear(),console.log(t+" "+r)},e+o*s)),o++}),e+=2*s,r>=e||!n?void setTimeout(function(){anim(t,e,r,s,i,e>r)},2*s):c())}function AI(rules){return this.rules=rules||[[0,"",null]],this.db=new DB("AI rules",["Index","Rules","Precision"],this.rules),this.db.init(),this.updateRule=function(t,e,r){var s=this.db.find(t)
...
for(var c=0;7>c;c++)n+="<td>"+a[c]+"</td>"
n+="</tr>"
for(var u=1;u<=Math.ceil((t+t-1)/7);u++){n+="<tr align='right' valign='top'>"
for(var l=1;7>=l&&!(o>t);l++)t>h?(n+="<td></td>",h++):(n+=o===r?"<td><span style='color: red'>"+o+"</span></td>":"<td>"+o+"</td>",o++)
