...
var n=r.pop()
try{if(n.payload===t)return[i,s]}catch(t){return[-1,s]}n.left&&r.push(n.left),n.right&&r.push(n.right),i++,s++}},this.bfs=function(t){var i=0,s=0,r=[]
for(r.unshift(this);r!=[];){i=0
var n=r.pop()
try{if(n.payload===t)return[i,s]}catch(t){return[-1,s]}n.left&&r.unshift(n.left),n.right&&r.unshift(n.right),i++,s++}},this.sum=function(){var t=this.payload
...
s+=r+t
try{r.left&&i.unshift(r.left),r.right&&i.unshift(r.right)}catch(i){Essence.say(i+" caused "+this+".printBFS("+t+") to go wrong","err")}}return t},this.toString=function(){var t="TreeNode(payload = "+this.payload+", "
...
return t?i.toString().split(","):i},this}function Set(t){return this.value=(isType(t,"Array")?rmDuplicates(t):[t])||[],this.size=function(){return this.value.length},this.add=function(t){if(isType(t,"array"))for(var i=0;i<t.length;i++)this.add(t[i])
this.value.indexOf(t)===-1&&this.value.push(t)},this.remove=function(t){if(this.value.has(t))if(isType(t,"array"))for(var i=0;i<t.length;i++)this.remove(t[i])
...
return this.top--,i}for(var s=0;s<t.length;s++)this.pop(t[s])},this.isEmpty=function(){return this.top<=-1},this.isFull=function(){return this.top>=this.value.length},this.size=function(){return this.top+1},this.toString=function(){return"Stack("+this.value.toString()+")"},this}function StackList(t){return this.top=null,this.ground=function(){return this.isEmpty()||null===this.top?null:null===this.top.next?this.top.payload:this.top.next.payload},this.peek=function(){return this.top.payload},this.push=function(t){if(isType(t,"array"))for(var i=0;i<t.length;i++)this.push(t[i])
...
return this.top=this.top.next,i}for(var s=0;s<t;s++)this.pop()},this.isEmpty=function(){return null==this.top},this.size=function(){for(var t=0,i=this.top;null!=i;)i=i.next,t++
...
this.back.prev=s,this.back=s,this.len++}return this},this.dequeue=function(){if(this.isEmpty())throw new Error("I can't dequeue an empty queue list")
...
return t},this.remove=function(t){for(var i=this.front;null!=i;)null==i.payload||i.payload!=t&&!i.payload.equals(t)||(i=null),i=i.next},this.insertAt=function(t,i){this.back.next=new Node(i),0===t?this.front=new Node(i):t===this.len?this.back=new Node(i):t>this.len&&this.enqueue(i)},this}function Astar(t,i){for(var s,r,n=i,e=[t],a=[];e.length>0;){var o=[],u=e[0].f
for(var l in e)e.hasOwnProperty(l)&&(o.push(e[l].f),u=Math.min(u,e[l].f))
if(s=e.filter(function(t){if(t.f===u)return t})[0],e=e.filter(function(t){if(t.f!=u)return t}),s===n){for(var f in s)if(s.hasOwnProperty(f)){f.f=s.f+h(f,s)
var c=lookfor(f,e)
if(c!=-1){if(c=c[0],s.f<e[c]||lookfor(f,a)!=-1&&s.f<e[lookfor(f,a)][0].f)continue
e=e.remove(f),a=a.remove(f),f.parent=s,r=h(f,n),e.push(f)}a.push(s)}throw"Solution found ! With h="+r}}}function A(t,i,s){for(var r=[],n=[t],e=[],h=[0],a=[euclidianDist(t,i)];n.length>0;){var o=n[a.indexOf(a.min())]
if(o===i)return reconPath(e,o,s)
n=n.remove(o),r.push(o)
var u=s.neighbour(o[0],o[1])
Essence.say("neighbour of "+o+":\n"+u.toStr(!0),"info")
for(var l=0;l<u;l++)if(!r.has(u[l])){var f=h[r.indexOf(o)]+1
if(r.indexOf(u[l])===-1)n.push(u[l])
...
s*=2,r=r<i?t[Math.floor(t.length/s)]:t[3*Math.floor(t.length/s)],debugging&&Essence.say("i="+s+"\nterm="+r+"\nloops="+n),n++}return r===i}function search(t,i){var s=0,r=t.length-1,n=Math.floor((r-s)/2)
if(t[n]===i)return n
for(;t[n]!=i;){if(r<s)return-1
...
i||(i=1)
for(var r=0;r<(t||1e3);r+=i)this.add({timestamp:s+r})},this.fill=function(t,i){var s=(new Date).getTime(),r=lookfor(s,this.table)
r===-1&&s>this.table.last()[2]?this.add({source:t,event:i,timestamp:s}):(this.table[r[0]][0]=t,this.table[r[0]][1]=i)},this.getCleanTable=function(){for(var t=[],i=0;i<this.table.length;i++)isNon(this.table[i][1])||t.push(this.table[i])
return t},this.lookAt=function(t){var i=lookfor(t||(new Date).getTime(),this.table)[0]
return"'"+this.table[i][1]+"' at "+this.table[i][0]},this}function Map(t){return this.keys=new SortedSet(t||[]),this.values=Tablify(this.keys.value),this.add=function(t,i){if(isType(t,"Array"))for(var s=0;s<t;s++)this.add(t[s],isType(i,"Array")?i[s]:i)
...
this.forEach(function(r){s.replace(r,t,i)})},this}function DomTree(){var t=document.children[0],i=new NTreeNode(t.tagName.toLowerCase(),t.children.toArray().map(function(t){return new NTreeNode(t.tagName.toLowerCase())}))
return i.add(),i}function DomStructure(){var t={},i=document.children[0],s=function(t){return t.localName||t.tagName.toLowerCase()},r=function(t){return t.children.toArray()},n=function(t,i){return i||(i={}),i[s(t)]={},r(t).map(function(e){i[s(t)][s(e)]={},r(e).isEmpty()||r(e).map(function(r){i[s(t)][s(e)]=n(r,i[s(t)][s(e)])})}),i}
return n(i,t)}var DataStruct=new Module("DataStruct","Data structures",["DOM","Maths","Files"])
