 ...
 */
function DomTree () {
	var current = document.children[0], nameOf = function (tag) { //Return the tag's name
		return tag.localName || tag.tagName.toLowerCase();
	}, kidsOf = function (tag) {
		return tag.children.toArray();
	}, stepIn = function (tag) {
		return kidsOf(tag).isEmpty()? new NTreeNode(nameOf(tag)): new NTreeNode(nameOf(tag), kidsOf(tag).map(function (child) {
			return kidsOf(child).isEmpty()? new NTreeNode(nameOf(child)): stepIn(child);
		}));
	};
