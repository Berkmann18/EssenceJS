...
function Perm (data) {
	if (data.length <= 1) return data;
		...
		for (var i = 1; i < data.length; i++) {
			perm.append(Perm(data.remove()).map(function (x) {
				return data[i] + x;
			}));
		}
...
function Map (keys) {
	...
	this.remove = function (key, value) {
		value? this.values[key] = this.values[key].remove(): this.keys.remove();
	};
	this.toString = function () {
...
var DOMA = (function (){
	...
	function kidsOfTag (tag) {
        return tag.children.toArray();
    }

    function printOrder (elm, t, s, d, sym) {
        if (!t) t = "";
        if (!s) s = "&nbsp;&nbsp;";
        if (!d) d = 0;
        if (!sym) sym = "|-";

        for (var child in elm) {
            if (elm.hasOwnProperty(child)) child.postOrder(t + s, s, d + 1, sym);
        }
 ...
 */
function DomTree () {
	var current = document.children[0], stepIn = function (tag) {
		if (debugging) Essence.say("Stepping in %c" + DOMA.nameOf(tag) + "%c", "info", "color: #00f", "color: #000");
		return DOMA.kidsOf(tag).isEmpty()? new NTreeNode(DOMA.nameOf(tag)): new NTreeNode(DOMA.nameOf(tag), DOMA.kidsOf(tag).map(function (child) {
...
function DomStructure () {
	var dom = {}, current = document.children[0], stepIn = function (tag, parent) {
		if (!parent) parent = {};
		parent[DOMA.nameOf(tag)] = {};
        DOMA.kidsOf(tag).map(function (child) {
			parent[DOMA.nameOf(tag)][DOMA.nameOf(child)] = {};
			if (!DOMA.kidsOf(child).isEmpty()) {
                DOMA.kidsOf(child).map(function (subChild) {
					//parent[nameOf(tag)][nameOf(child)][nameOf(subChild)] = {};
					parent[DOMA.nameOf(tag)][DOMA.nameOf(child)] = stepIn(subChild, parent[DOMA.nameOf(tag)][DOMA.nameOf(child)]);
				})
			}
		});
		return parent;
 ...
 */
function DomList () {
	var res = "", s = "  ", space = "", dom = {}, current = document.children[0], stepIn = function (tag, parent, spacing, depth) {
        if (!parent) parent = {};
        parent[DOMA.nameOf(tag)] = {};
        ...
        DOMA.kidsOf(tag).map(function (child) {
            ...
            if (!DOMA.kidsOf(child).isEmpty()) {
                DOMA.kidsOf(child).map(function (subChild) {
                    //parent[nameOf(tag)][nameOf(child)][nameOf(subChild)] = {};
                    parent[DOMA.nameOf(tag)][DOMA.nameOf(child)] = stepIn(subChild, parent[DOMA.nameOf(tag)][DOMA.nameOf(child)], space + s + s, depth + 2);
                    //res += spacing + s + s + ... + "\n";
                })
            }
        });
 ...
 */
function object2tree (obj, space, symbol, forDOM) {
	if (!space) space = forDOM? "&nbsp;&nbsp;": "\t";
	if (!symbol) symbol = "|-";

	var treeOf = function (obj) {
		return space + symbol + keyList(obj, true) + (forDOM? "<br />": "\n");
	};
	var tree = treeOf(obj);
	console.log("tree", tree);
	for (var elm in obj) {
		if (obj.hasOwnProperty(elm)) {
			//console.log("tree from " + elm + "'s view: " + treeOf(obj[elm]));
			tree += /*treeOf(obj[elm]); */object2tree(obj[elm], space + space, symbol, forDOM);
        }
	}
	return tree;
}
