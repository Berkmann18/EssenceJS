...
return e.mult(2*this.dot(t||this.normal)),this.sub(e),this},this.angle=function(t){return Math.acos((this.x*t.x+this.y*t.y)/(this.length()*t.length()))},this}function Polygon(t,e,i){this.points=t,this.border=e||1,this.vel=i||new Vector,this.norm=this.vel.getNormal()
var r=this
...
return r+"</select>"+n+"</select>"+s+"</select>"}function initCanvas(t,e){t||(t=500),e||(e=500),null===$n("canvas#essenceCanvas",!0)?print("<canvas id='essenceCanvas' width='"+t+"' height='"+e+"'>Canvas isn't supported by this browser</canvas>",!0):($n("canvas#essenceCanvas").width=t,$n("canvas#essenceCanvas").height=e)}function runCanvas(t,e,i){i||(i=0),t($n("canvas#essenceCanvas").getContext(e||"2d")),1>i&&runCanvas(t,e,i+1)}var UI=new Module("UI","UI stuff",["Maths"])
