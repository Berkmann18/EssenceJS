...
break
default:r.scroll(0,1),r.node.scrollTop===r.node.scrollHeight-r.node.offsetTop&&clearInterval(i)}},e||50)},this.attr=function(t,e){return isNon(e)?this.node.getAttribute(t):this.node.setAttribute(t,e)},this.rmAttr=function(t){this.node.removeAttribute(t)},this.invColour=function(){negateColour(t,"color","a"),negateColour(t,"backgroundColor","a")},this}function include(t,e){e||(e=t.indexOf(".js")>0?"script":"link")
...
e.innerText=t,e.type="text/css",e.id="EssenceCSS",$n("head").appendChild(e)},addJS:function(t){var e=document.createElement("script")
...
return e},String.prototype.map=function(t,e){return this.split(e||"").map(t).join(e||"")},String.prototype.reverse=function(t){return this.split(t||"").reverse().join(t||"")},String.prototype.minify=function(t){var e=this.trim().replace(/(\t|\n|\s{2,})/gm,"")
return t?e.replace(/(<!--(.*?)-->|\/\*+(.*?)\*+\/)/gm,""):e},Number.prototype.length=function(){if(String(this).has("."))return[parseInt(String(this).split(".")[0].length),parseInt(String(this).split(".")[1].length)]
